  ...
  return (
    <div className="max-w-6xl mx-auto">
      <div className="mb-8">
        ...
        <p className="text-slate-600">Track and manage cyber fraud complaints and voice analysis cases</p>
      </div>

      <div className="bg-white rounded-lg shadow-sm border border-slate-200 p-6 mb-6">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div className="text-center">
            <div className="text-2xl font-bold text-blue-600">{mockComplaints.length}</div>
            <div className="text-sm text-slate-600">Total Complaints</div>
          </div>
          <div className="text-center">
            <div className="text-2xl font-bold text-yellow-600">
              {mockComplaints.filter(c => c.status === 'investigating').length}
            </div>
            <div className="text-sm text-slate-600">Under Investigation</div>
          </div>
          <div className="text-center">
            <div className="text-2xl font-bold text-red-600">
              {mockComplaints.filter(c => c.priority === 'critical').length}
            </div>
            <div className="text-sm text-slate-600">Critical Priority</div>
          </div>
          <div className="text-center">
            <div className="text-2xl font-bold text-green-600">
              {formatCurrency(mockComplaints.reduce((sum, c) => sum + c.estimatedLoss, 0))}
            </div>
      ...
      <div className="space-y-4">
        {mockComplaints.map((complaint) => (
          <ExpandableListItem
            key={complaint.id}
            title={`${complaint.caseNumber} - ${complaint.title}`}
            subtitle={`Reported by ${complaint.reportedBy} • ${formatDate(complaint.reportedDate)}`}
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-3 text-sm">
                  ...
                  <div className="flex items-center">
                    ...
                    <span className={`ml-2 px-2 py-1 text-xs font-medium rounded-full ${getPriorityColor(complaint.priority)}`}>
                  ...
                  <div className="flex items-center">
                    ...
                    <span className="ml-2 text-slate-900">{formatFraudType(complaint.fraudType)}</span>
                  </div>
                  <div className="flex items-center">
                    <User className="h-4 w-4 text-slate-400 mr-3" />
                    <span className="text-slate-600">Assigned To:</span>
                    <span className="ml-2 text-slate-900">{complaint.assignedTo}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-3 text-sm">
                  <div className="flex items-center">
                    ...
                    <span className="ml-2 text-slate-900 font-medium">{complaint.voiceSamples}</span>
                  </div>
                  <div className="flex items-center">
                    <DollarSign className="h-4 w-4 text-slate-400 mr-3" />
                    <span className="text-slate-600">Estimated Loss:</span>
                    <span className="ml-2 text-slate-900 font-medium">{formatCurrency(complaint.estimatedLoss)}</span>
                  </div>
                  <div className="flex items-center">
                    <MapPin className="h-4 w-4 text-slate-400 mr-3" />
                    <span className="text-slate-600">Location:</span>
                    <span className="ml-2 text-slate-900">{complaint.location}</span>
                  </div>
                  <div className="flex items-center">
                ...
                <div>
                  <div className="flex items-center mb-2">
                    ...
                    <span className="text-slate-600 font-medium text-sm">Evidence Files:</span>
                  </div>
